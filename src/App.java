...
public class App  {
	...
	public void getRegisteredUsersToString(){
		var ref = new Object() {
			int i = 1;
		};
		registeredUsers.forEach((key, value) -> {
			if(!value.getName().equals(getCurrentUser().getName()) && this.currentUser != value) {
				System.out.println(ref.i + ". Username: " + value.getName() + "; \n   E-mail: " + key + ";");
				ref.i++;
			}
		});
	}
	public void getRegisteredUserPosts(){
		registeredUsers.forEach((key, value) -> {
			System.out.print(value.getName());
			System.out.println(value.getMyWall().toString());
		});
	}
